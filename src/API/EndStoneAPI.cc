...
namespace jse
{
    ClassDefine<void> EndStoneAPIClass =
        defineClass("endstone")
            .function("registerPlugin", &EndStoneAPI::registerPlugin)
            .build();
    ...
    Local<Value> EndStoneAPI::registerPlugin(Arguments const &args)
    {
        ...
        try
        {
            ...
            else
            {
                ...
                GetEntry()->getLogger().warning("Plugin name is not specified, use file name as plugin name, file name: {}", data->mFileName);
            }
